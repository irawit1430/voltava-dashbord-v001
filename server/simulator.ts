...
export function addGateway(data: any): Gateway {
  ...
  if (data.connectionType === 'tcp') {
    ...
    const duplicate = gateways.find(g => g.connectionType === 'tcp' && g.ipAddress === data.ipAddress && g.port === Number(data.port));
    if (duplicate) {
      throw new Error(`Port ${data.port} on IP ${data.ipAddress} is already assigned to another gateway`);
    }
  } else if (data.connectionType === 'serial') {
    if (!data.serialPort || typeof data.serialPort !== 'string' || data.serialPort.trim() === '') {
...
export function updateGateway(id: string, data: Partial<Gateway>): Gateway | null {
  ...
  const newConnectionType = data.connectionType !== undefined ? data.connectionType : existing.connectionType;
  const newIpAddress = data.ipAddress !== undefined ? data.ipAddress : existing.ipAddress;
  const newPort = data.port !== undefined ? Number(data.port) : existing.port;
  const newSerialPort = data.serialPort !== undefined ? data.serialPort : existing.serialPort;
  const newBaudRate = data.baudRate !== undefined ? Number(data.baudRate) : existing.baudRate;
  ...
  if (newConnectionType === 'tcp') {
    ...
    const duplicate = gateways.find(g => g.id !== id && g.connectionType === 'tcp' && g.ipAddress === newIpAddress && g.port === newPort);
    if (duplicate) {
      throw new Error(`Port ${newPort} on IP ${newIpAddress} is already assigned to another gateway`);
    }
  } else if (newConnectionType === 'serial') {
    if (!newSerialPort || typeof newSerialPort !== 'string' || newSerialPort.trim() === '') {
      throw new Error('Serial Port is required for Serial connection');
    }
    if (!newBaudRate) {
      throw new Error('Baud Rate is required for Serial connection');
