...
describe('DeviceList Component', () => {
  const mockDevices: Device[] = [
    ...
    },
    {
      ...
      model: 'SOL-500', firmware: 'v1.0',
      type: 'solar',
      status: 'warning',
      owner: 'EcoPower',
      location: { lat: 0, lng: 0, city: 'Mumbai' },
      telemetry: { voltage: 220, current: 15, temp: 50, activePower: 3.3, soc: 0, soh: 100, lastUpdated: '12:00', faults: ['Overvoltage Warning'] }
    },
    {
      id: 'dev-3',
      name: 'BESS Controller Gamma',
      model: 'BES-1000', firmware: 'v1.0',
      type: 'bess',
      status: 'fault',
      owner: 'Voltava',
      location: { lat: 0, lng: 0, city: 'Delhi' },
      telemetry: { voltage: 400, current: 0, temp: 75, activePower: 0, soc: 20, soh: 95, lastUpdated: '12:00', faults: ['Thermal Runaway Protocol'] }
    }
  ];
  ...
  it('renders device table with all devices initially', () => {
    render(<DeviceList devices={mockDevices} onSelectDevice={mockOnSelectDevice} />);
    ...
    expect(screen.getByText('Found 3 matching energy assets')).toBeInTheDocument();
  });

  it('filters devices by search term', () => {
    render(<DeviceList devices={mockDevices} onSelectDevice={mockOnSelectDevice} />);

    const searchInput = screen.getByPlaceholderText(/Search by ID, name, owner.../i);
    fireEvent.change(searchInput, { target: { value: 'Alpha' } });

    expect(screen.getByText('Smart BMS Alpha')).toBeInTheDocument();
    expect(screen.queryByText('Solar Inverter Beta')).not.toBeInTheDocument();
    expect(screen.getByText('Found 1 matching energy assets')).toBeInTheDocument();
  });
