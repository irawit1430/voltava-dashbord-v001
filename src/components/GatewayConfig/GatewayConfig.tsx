...
export default function GatewayConfig({
  ...
  const [activeDiagnosticTab, setActiveDiagnosticTab] = useState<'ping' | 'scan' | 'packets'>('ping');
  
  // Terminal logs
  const [pingLog, setPingLog] = useState<string>('Select a gateway and click "Run Ping Test"\n');
  const [scanLog, setScanLog] = useState<string>('Select a gateway and click "Scan Bus"\n');
  ...
  // Real-time packet generator via WebSocket
  useEffect(() => {
    if (!selectedGateway || selectedGateway.status !== 'online' || activeDiagnosticTab !== 'packets') {
      setPacketLog([]);
      return;
    }

    setPacketLog([`[SYS] Packet Analyzer session started for ${selectedGateway.name} (${selectedGateway.id})`]);
    ...
    const connectWs = () => {
      ...
      const wsUrl = window.location.port !== '' && window.location.port !== '5000'
        ? `${wsProtocol}//${window.location.hostname}:5000/ws/packets?gatewayId=${selectedGateway.id}`
        : `${wsProtocol}//${window.location.host}/ws/packets?gatewayId=${selectedGateway.id}`;
      ...
      ws.onmessage = (event) => {
        try {
          ...
          if (message.type === 'PACKET' && message.data) {
            setPacketLog(prev => [message.data, ...prev].slice(0, 25));
          }
        } catch (err) {
          console.error('Error parsing packet message:', err);
        }
      };

      ws.onerror = (err) => {
        console.error('Packet WS error:', err);
      };

      ws.onclose = () => {
        if (active) {
          // Reconnect logic if needed, but for diagnostics we might just let it die or retry
  ...
  const handleRunPing = async () => {
    ...
    setIsPinging(true);
    setPingLog(`Voltava Diagnostics: Pinging gateway ${selectedGateway.id}...\n`);
  ...
  const handleRunScan = async () => {
    if (!selectedGateway) return;
    setIsScanning(true);
    setScanLog(`Voltava Diagnostics: Initiating bus scan on ${selectedGateway.id}...\n`);
  ...
  const getProtocolLabel = (proto: GatewayProtocol) => {
    switch (proto) {
      ...
      case 'dlms': return 'DLMS/COSEM';
      case 'ocpp': return 'OCPP 1.6J';
      case 'can': return 'CAN-Bus (J1939)';
  ...
  const packetsSec = gateways.reduce((acc, curr) => acc + (curr.status === 'online' ? Math.floor(1 / curr.pollingInterval * 10) / 10 : 0), 0).toFixed(1);
  const successRate = totalGateways > 0 
    ? Math.round(
        (gateways.reduce((acc, curr) => acc + curr.packetsTransmitted, 0) / 
        Math.max(1, gateways.reduce((acc, curr) => acc + curr.packetsTransmitted + curr.packetsFailed, 0))) * 100
      ) 
    : 100;
      ...
      <div className="gw-stats-grid">
        <div className="glass-panel gw-stat-card">
          <div className="gw-stat-header">
            ...
            <div className="gw-icon-wrap" style={{ backgroundColor: 'rgba(6, 182, 212, 0.1)' }}>
              <Settings size={18} color="var(--accent-cyan)" />
            </div>
          </div>
          <div className="gw-stat-value">{totalGateways}</div>
          <div className="gw-stat-sub-text">{onlineGateways} Online actively routing</div>
        </div>

        <div className="glass-panel gw-stat-card">
          <div className="gw-stat-header">
            <span className="gw-stat-label">Routing Frequency</span>
            <div className="gw-icon-wrap" style={{ backgroundColor: 'rgba(16, 185, 129, 0.1)' }}>
              <Activity size={18} color="var(--accent-green)" />
            </div>
          </div>
          <div className="gw-stat-value">{packetsSec} <span style={{ fontSize: '1rem', color: 'var(--text-muted)' }}>pkts/s</span></div>
        ...
        <div className="glass-panel gw-stat-card">
          <div className="gw-stat-header">
            ...
            <div className="gw-icon-wrap" style={{ backgroundColor: 'rgba(59, 130, 246, 0.1)' }}>
        ...
        <div className="glass-panel gw-list-section">
          ...
          <div className="gw-list-container">
            {gateways.map(g => {
              ...
              const connInfo = g.connectionType === 'tcp' 
                ? `${g.ipAddress}:${g.port}`
                : `${g.serialPort}`;
              ...
              return (
                ...
                >
                  <div className="gw-row-main">
                    <div className="gw-row-info">
                      ...
                      <span className="gw-row-meta">
                        <span className="gw-row-proto">{getProtocolLabel(g.protocol)}</span>
                    ...
                    <div className="gw-row-status">
                      <span className={`badge ${
                        g.status === 'online' ? 'badge-online' :
                        g.status === 'connecting' ? 'badge-warning' : 'badge-fault'
                  ...
                  <div className="gw-row-footer">
                    <span>Tx: {g.packetsTransmitted} | Err: {g.packetsFailed}</span>
        ...
        <div className="gw-detail-section">
          {isAdding ? (
            ...
            <div className="glass-panel gw-form-container">
              ...
              <form onSubmit={handleAddGateway} className="gw-form">
                <div className="gw-form-grid">
                  <div className="gw-form-group">
                    ...
                    <input 
                      ...
                      value={newGw.name}
                      onChange={(e) => setNewGw(prev => ({ ...prev, name: e.target.value }))}
                      className="gw-input"
                    />
                  </div>
                  ...
                  <div className="gw-form-group">
                    ...
                    <select
                      ...
                      onChange={(e) => setNewGw(prev => ({ ...prev, protocol: e.target.value as GatewayProtocol }))}
                      className="gw-select"
                    >
                  ...
                  <div className="gw-form-group">
                    ...
                    <select
                      ...
                      onChange={(e) => setNewGw(prev => ({ ...prev, connectionType: e.target.value as 'tcp' | 'serial' }))}
                      className="gw-select"
                    >
                      <option value="tcp">TCP/IP Network</option>
                      <option value="serial">Serial Port (RS-485 / RS-232)</option>
                    </select>
                  </div>

                  <div className="gw-form-group">
                    <label className="gw-label">Polling Interval (seconds)</label>
                    <input 
                      type="number" 
                      min="1" 
                      max="300"
                      value={newGw.pollingInterval}
                      onChange={(e) => setNewGw(prev => ({ ...prev, pollingInterval: Number(e.target.value) }))}
                      className="gw-input"
                    />
                  </div>
                    ...
                    <>
                      <div className="gw-form-group">
                        ...
                        <input 
                          ...
                          onChange={(e) => setNewGw(prev => ({ ...prev, ipAddress: e.target.value }))}
                          className="gw-input"
                        />
                      </div>
                      <div className="gw-form-group">
                        ...
                        <input 
                          ...
                          placeholder="502" 
                          value={newGw.port}
                          onChange={(e) => setNewGw(prev => ({ ...prev, port: Number(e.target.value) }))}
                          className="gw-input"
                        />
                      </div>
                    </>
                    ...
                    <>
                      <div className="gw-form-group">
                        ...
                        <input 
                          ...
                          onChange={(e) => setNewGw(prev => ({ ...prev, serialPort: e.target.value }))}
                          className="gw-input"
                        />
                      </div>
                      <div className="gw-form-group">
                        ...
                        <select
                          ...
                          onChange={(e) => setNewGw(prev => ({ ...prev, baudRate: Number(e.target.value) }))}
                          className="gw-select"
                        >
                        ...
                        <input 
                          ...
                          onChange={(e) => {
                            if (e.target.checked) {
                              ...
                              setNewGw(prev => ({ ...prev, connectedDevices: prev.connectedDevices.filter(id => id !== d.id) }));
                            }
                          }}
                          style={{ marginRight: 8 }}
                        />
                ...
                <div className="gw-form-actions">
                  <button type="button" onClick={() => setIsAdding(false)} className="gw-cancel-btn">Cancel</button>
                  <button type="submit" className="gw-submit-btn">Create Node</button>
                </div>
                {error && <div style={{ color: 'var(--accent-red)', fontSize: '0.8rem', marginTop: '0.5rem', textAlign: 'right' }}>{error}</div>}
              </form>
            </div>
          ) : selectedGateway ? (
            ...
            <div className="gw-detail-wrapper">
              ...
              <div className="glass-panel gw-detail-card">
                <div className="gw-detail-card-header">
                  ...
                  <div className="gw-detail-actions">
                    ...
                    <button 
                      ...
                      style={{
                        borderColor: selectedGateway.status === 'online' ? 'rgba(16, 185, 129, 0.4)' : 'rgba(239, 68, 68, 0.4)',
                        backgroundColor: selectedGateway.status === 'online' ? 'rgba(16, 185, 129, 0.08)' : 'rgba(239, 68, 68, 0.08)',
                        color: selectedGateway.status === 'online' ? 'var(--accent-green)' : 'var(--accent-red)'
                      }}
                    >
                ...
                {isEditing ? (
                  ...
                  <form onSubmit={handleEditGateway} className="gw-form" style={{ padding: '1rem 0 0 0' }}>
                    <div className="gw-form-grid">
                      <div className="gw-form-group">
                        <label className="gw-label">Gateway Name</label>
                        <input 
                          ...
                          required
                          value={editGw.name}
                          onChange={(e) => setEditGw(prev => ({ ...prev, name: e.target.value }))}
                          className="gw-input"
                        />
                      </div>
                      ...
                      <div className="gw-form-group">
                        ...
                        <input 
                          ...
                          onChange={(e) => setEditGw(prev => ({ ...prev, pollingInterval: Number(e.target.value) }))}
                          className="gw-input"
                        />
                      </div>
                        ...
                        <>
                          <div className="gw-form-group">
                            ...
                            <input 
                              ...
                              onChange={(e) => setEditGw(prev => ({ ...prev, ipAddress: e.target.value }))}
                              className="gw-input"
                            />
                          </div>
                          <div className="gw-form-group">
                            ...
                            <input 
                              type="number" 
                              value={editGw.port}
                              onChange={(e) => setEditGw(prev => ({ ...prev, port: Number(e.target.value) }))}
                              className="gw-input"
                            />
                          </div>
                        </>
                        ...
                        <>
                          <div className="gw-form-group">
                            ...
                            <input 
                              ...
                              onChange={(e) => setEditGw(prev => ({ ...prev, serialPort: e.target.value }))}
                              className="gw-input"
                            />
                          </div>
                          <div className="gw-form-group">
                            ...
                            <select
                              ...
                              onChange={(e) => setEditGw(prev => ({ ...prev, baudRate: Number(e.target.value) }))}
                              className="gw-select"
                            >
                              <option value="2400">2400 bps</option>
                              <option value="4800">4800 bps</option>
                    ...
                    <div className="gw-form-actions">
                      <button type="button" onClick={() => setIsEditing(false)} className="gw-cancel-btn">Cancel</button>
                      <button type="submit" className="gw-submit-btn">Save Configuration</button>
                    </div>
                      {error && <div style={{ color: 'var(--accent-red)', fontSize: '0.8rem', marginTop: '0.5rem', textAlign: 'right' }}>{error}</div>}
                  </form>
                ) : (
                  ...
                  <div className="gw-meta-row-grid">
                    <div className="gw-meta-col">
                      ...
                      <span className="gw-meta-val">{getProtocolLabel(selectedGateway.protocol)}</span>
                    </div>

                    <div className="gw-meta-col">
                      <span className="gw-meta-label">Medium</span>
                      <span className="gw-meta-val">{selectedGateway.connectionType.toUpperCase()}</span>
                    </div>
                    ...
                    {selectedGateway.connectionType === 'tcp' ? (
                      <>
                        <div className="gw-meta-col">
                          ...
                          <span className="gw-meta-val">{selectedGateway.ipAddress}</span>
                        </div>
                        <div className="gw-meta-col">
                          <span className="gw-meta-label">TCP Port</span>
                          <span className="gw-meta-val">{selectedGateway.port}</span>
                        </div>
                      </>
                    ) : (
                      <>
                        <div className="gw-meta-col">
                          ...
                          <span className="gw-meta-val">{selectedGateway.serialPort}</span>
                        </div>
                        <div className="gw-meta-col">
                          <span className="gw-meta-label">Baud Rate</span>
                          <span className="gw-meta-val">{selectedGateway.baudRate} bps</span>
                        </div>
                      </>
                    )}
                    ...
                    <div className="gw-meta-col">
                      ...
                      <span className="gw-meta-val">{selectedGateway.pollingInterval}s</span>
                    </div>
                  </div>
                )}
                ...
                <div className="gw-linked-section">
                  <h4 className="gw-sub-header">Monitored Assets Connected via Gateway</h4>
                  <div className="gw-linked-grid">
                      ...
                      return (
                        <div key={dev.id} className="gw-linked-device-card">
                          ...
                          <div className="gw-dev-card-footer">
                            <span className="gw-dev-card-model">{dev.model}</span>
                            <div style={{ display: 'flex', alignItems: 'center', gap: 4 }}>
                              <span style={{
                                width: 6, height: 6, borderRadius: '50%',
                                backgroundColor: dev.status === 'online' ? 'var(--accent-green)' :
                                                 dev.status === 'warning' ? 'var(--accent-orange)' : 'var(--accent-red)'
                              }} />
                              <span style={{ fontSize: '0.65rem', textTransform: 'uppercase', fontWeight: 600 }}>{dev.status}</span>
                            </div>
                          </div>
                        </div>
                      );
                    ...
                    {selectedGateway.connectedDevices.length === 0 && (
                      <div className="gw-empty-linked">
                        <AlertTriangle size={14} color="var(--accent-orange)" style={{ marginRight: 6 }} />
                        <span>No assets linked to this gateway node. Edit settings to link devices.</span>
                      </div>
                    )}
                  </div>
                </div>
              ...
              <div className="glass-panel gw-terminal-panel">
                ...
                <div className="gw-terminal-action-bar">
                  <span className="gw-terminal-title">
                    ...
                    {activeDiagnosticTab === 'scan' && 'Industrial Bus Address Scan'}
                    {activeDiagnosticTab === 'packets' && 'Live RTU/TCP Packet Sniffer'}
                  </span>
                  ...
                  {activeDiagnosticTab === 'packets' && (
                    <div style={{ display: 'flex', alignItems: 'center', gap: 6, fontSize: '0.7rem', color: 'var(--text-muted)' }}>
                      <span style={{ 
                        ...
                        backgroundColor: selectedGateway.status === 'online' ? 'var(--accent-green)' : 'var(--accent-red)'
                      }} />
                      {selectedGateway.status === 'online' ? 'Sniffing active...' : 'Sniffer inactive. Gateway is offline.'}
                    </div>
                  )}
                </div>
            ...
            <div className="gw-empty-detail">
              <span>Select a gateway from the list to view configuration and diagnostics.</span>
            </div>
          )}
        </div>
