...
export default function Navbar({ activeTab, devices, onSelectDevice }: NavbarProps) {
  ...
  useEffect(() => {
    ...
    if (prevDevices.length === 0 && devices.length > 0) {
      ...
      const initialAlerts = devices.filter(d => d.status === 'fault' || d.status === 'warning');
      const initialNotifs: AppNotification[] = initialAlerts.map(device => ({
        id: crypto.randomUUID(),
        deviceId: device.id,
        deviceName: device.name,
        status: device.status as 'fault' | 'warning',
        message: device.telemetry.faults && device.telemetry.faults.length > 0
      ...
      devices.forEach(currentDevice => {
        ...
        if (prevDevice) {
          ...
          if ((currentDevice.status === 'fault' || currentDevice.status === 'warning') &&
              (prevDevice.status !== 'fault' && prevDevice.status !== 'warning')) {
            newNotifications.push({
              id: crypto.randomUUID(),
              deviceId: currentDevice.id,
              deviceName: currentDevice.name,
              status: currentDevice.status as 'fault' | 'warning',
              message: currentDevice.telemetry.faults && currentDevice.telemetry.faults.length > 0
          ...
          else if ((prevDevice.status === 'fault' || prevDevice.status === 'warning') &&
    ...
    <header className="navbar glass-panel">
      ...
      <div className="right-controls">
        ...
        <div style={{ position: 'relative' }} ref={dropdownRef} onKeyDown={handleKeyDown}>
          ...
          {isOpen && (
            <div className="popover glass-panel notification-popover" role="dialog" aria-label="System Notifications">
              ...
              <div className="popover-list" role="list">
                {notifications.length === 0 ? (
                  <div className="empty-state">
                    ...
                    <p className="empty-sub-text">No active alerts or recent events.</p>
                  </div>
                ) : (
                    ...
                    return (
                      <div
                        ...
                        className={`notification-item notification-item-hover ${
                          isFault ? 'notification-item-fault' :
                          isWarning ? 'notification-item-warning' :
                        ...
                        onClick={() => {
                          ...
                          const deviceToSelect = devices.find(d => d.id === notif.deviceId);
                          if (deviceToSelect) {
                      ...
                      >
                        <div className="notification-icon-wrap">
                          {isFault && <AlertTriangle size={16} color="var(--accent-red)" className="animate-pulse" />}
                          {isWarning && <AlertTriangle size={16} color="var(--accent-orange)" />}
                          {isResolved && <CheckCircle2 size={16} color="var(--accent-green)" />}
                        </div>
                        <div className="notification-content">
                          <div className="notification-meta">
                            <span className="device-name">{notif.deviceName}</span>
                          ...
                          <div className="notification-footer">
                            <span className="device-id-text">
                              {notif.timestamp.toLocaleTimeString([], {hour: '2-digit', minute:'2-digit'})} • {notif.deviceId.substring(0, 8)}
                            </span>
                          </div>
                        </div>
                        <button
